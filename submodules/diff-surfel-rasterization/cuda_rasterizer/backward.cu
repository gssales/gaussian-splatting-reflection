...
__device__ void computeColorFromSH4(int idx, int deg, int max_coeffs, const glm::vec3* means, glm::vec3 campos, const float* shs, const bool* clamped, const glm::vec4* dL_dcolor, glm::vec3* dL_dmeans, glm::vec4* dL_dshs)
{
	// Compute intermediate values, as it is done during forward
	glm::vec3 pos = means[idx];
	glm::vec3 dir_orig = pos - campos;
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			if (!unstable) {
				...
				s = {p.x * inv_pz, p.y * inv_pz};
			} else {
				s = {0.0f, 0.0f};  // dummy, won't be used
			}
	...
	glm::vec4* dL_drots)
{
	...
	if (shs)
		computeColorFromSH4(idx, D, M, (glm::vec3*)means3D, *campos, shs, clamped, (glm::vec4*)dL_dcolors, (glm::vec3*)dL_dmean3Ds, (glm::vec4*)dL_dshs);
