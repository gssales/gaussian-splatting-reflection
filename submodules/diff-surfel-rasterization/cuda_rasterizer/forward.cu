...
__device__ glm::vec4 computeColorFromSH4(int idx, int deg, int max_coeffs, const glm::vec3* means, glm::vec3 campos, const float* shs, bool* clamped)
{
	// The implementation is loosely based on code for 
	// "Differentiable Point-Based Radiance Fields for 
	// Efficient View Synthesis" by Zhang et al. (2022)
	...
	bool prefiltered)
{
	...
	if (colors_precomp == nullptr) {
		glm::vec4 result = computeColorFromSH4(idx, D, M, (glm::vec3*)orig_points, *cam_pos, shs, clamped);
